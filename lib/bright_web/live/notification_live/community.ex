defmodule BrightWeb.NotificationLive.Community do
  ...
  def render(assigns) do
      ...
      <div class="text-sm font-medium text-center">
        ...
        <%= for notification <- @notifications do %>
          <li class="flex flex-wrap my-5">
            <div phx-click="confirm_notification" phx-value-notification_community_id={notification.id} class="cursor-pointer hover:filter hover:brightness-[80%] text-left flex flex-wrap items-center text-base px-1 py-1 flex-1 mr-4 w-full lg:w-auto lg:flex-nowrap truncate">
            ...
            <div class="flex gap-x-2 w-full justify-end lg:justify-start lg:w-auto">
              <button phx-click="confirm_notification" phx-value-notification_community_id={notification.id} class="hidden hover:filter hover:brightness-[80%] font-bold lg:inline-block bg-brightGray-900 text-white min-w-[76px] rounded p-2 text-sm">
                内容を見る
              </button>
            </div>
          </li>
    ...
    <.bright_modal
      ...
      <%!-- コミュニティのお知らせは運営が入力するため XSS のリスクはないとして raw を許容する --%>
