defmodule BrightWeb.OnboardingLive.WantToDoComponents do
        ...
        <div id="wants_todo_panel" class="hidden">
          <ul class="flex flex-wrap gap-4 justify-start p-4">
            ...
            <li class="bg-white px-4 py-4 rounded select-none w-72 hover:opacity-50">
              <a href={"/onboardings/select_skill_panel/#{wants.id}"} class="block">
  ...
  def mount(socket) do
    career_wants =
      Jobs.list_career_wants()
      |> Repo.preload(skill_panels: :career_field)
      |> Enum.map(fn wants ->
        Map.put(
          wants,
          :skill_panels,
          Enum.map(wants.skill_panels, & &1.career_field) |> Enum.uniq()
        )
      end)
