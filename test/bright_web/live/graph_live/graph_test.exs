defmodule BrightWeb.GraphLive.GraphsTest do
  ...
  describe "Show" do
    ...
    test "shows star", %{
      ...
      assert html =~ ~s{class="bg-white border border-brightGreen-500 rounded px-1 h-8 flex items-center mt-auto mb-1 hover:filter hover:brightness-95"}

      html = show_live
      |> element(~s{button[phx-click="click_star_button"]})
      |> render_click()

      assert html =~ ~s{class="bg-white border border-brightGreen-300 rounded px-1 h-8 flex items-center mt-auto mb-1 hover:filter hover:brightness-95"}
