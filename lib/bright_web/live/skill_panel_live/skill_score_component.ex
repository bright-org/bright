defmodule BrightWeb.SkillPanelLive.SkillScoreComponent do
  ...
  def render(assigns) do
    ...
    <div id={@id} class="flex justify-center gap-x-4 px-4">
      <div class="flex items-center">
        <%= if @edit do %>
          <div
            class={["cursor-pointer", score_mark_class(@skill_score)]}
            phx-click="input"
            phx-target={@myself}
          />
        <% else %>
  ...
  def handle_event("submit", %{"score" => score}, socket) do
    notify_score_change(socket.assigns.skill_score, String.to_atom(score))
  ...
  def handle_event("shortcut_key", %{"key" => key}, socket) when key in ~w(1 2 3) do
    ...
    notify_score_change(socket.assigns.skill_score, score)
  ...
  defp notify_score_change(skill_score, score) do
    send(self(), {__MODULE__, {:score_change, skill_score, score}})
  end
