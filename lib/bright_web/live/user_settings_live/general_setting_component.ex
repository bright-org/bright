      ...
      >
        <div class="border-b border-brightGray-200 flex flex-col lg:flex-row lg:flex-wrap text-left">
          <div class="w-full lg:w-1/2">
            <label class="border-b border-brightGray-200 flex items-center py-4">
              ...
              <BrightCore.input
                ...
                type="text"
                size="20"
                input_class="px-2 py-1 rounded w-60"
              />
            </label>
            <label class="border-b border-brightGray-200 flex items-center py-4">
            ...
            <label class="flex items-center pt-4 pb-2">
              ...
              <.inputs_for :let={ff} field={f[:user_profile]}>
                <BrightCore.input
                  ...
                  type="text"
                  size="20"
                  input_class="px-2 py-1 rounded w-60 placeholder-brightGray-100"
            ...
            <label class="flex items-center py-2">
              ...
              <.inputs_for :let={ff} field={f[:user_profile]}>
                <BrightCore.input
                  field={ff[:twitter_url]}
                  type="text"
                  size="20"
                  error_class="w-60 break-words"
                  input_class="px-2 py-1 rounded w-60 placeholder-brightGray-100"
          ...
          <div class="w-full flex lg:relative py-4 lg:w-1/2">
            <p>アイコン</p>
            <.error :for={err <- upload_errors(@uploads.icon)}>
              <%= upload_error_to_string(err) %>
            </.error>
            <%= for entry <- @uploads.icon.entries do %>
              <.error :for={err <- upload_errors(@uploads.icon, entry)}>
                <%= upload_error_to_string(err) %>
              </.error>
            <% end %>
            <.inputs_for :let={ff} field={f[:user_profile]}>
              <label
                ...
                class={[
                  ...
                  (!uploaded?(@uploads) || upload_error?(@uploads)) && !has_icon?(ff) &&
                    "bg-bgAddAvatar"
                ]}
              >
          ...
          <label class="flex py-4 w-full">
            ...
            <.inputs_for :let={ff} field={f[:user_profile]}>
              <BrightCore.input
                type="textarea"
                field={ff[:detail]}
                div_class="flex-1 w-full"
                input_class="flex-1 w-full"
                rows="3"
                cols="20"
              />
            </.inputs_for>
        ...
        <div class="flex mt-8 relative">
          <button
            type="submit"
            class="bg-brightGray-900 border block border-solid border-brightGray-900 cursor-pointer font-bold mx-auto px-4 py-2 rounded select-none text-center text-white w-80 hover:opacity-50"
          >
            保存する
          </button>
        </div>
