...
> Ｕ－ｅ）成長パネル：スキルパネル毎に、スキル成長（時系列）とスキルジェムを、過去履歴（3ヶ月毎、マスタも込み）／未来（≒スキルアップ対象）で切り替えて参照できる ※3ヶ月自動〆
>　→スキルセットに対しても上記同様の参照ができる
>　→システムの性能面が許せば、成長パネルだけで無く、スキルパネルの過去履歴（3ヶ月毎、マスタも込み）も参照可能とする
...
- ユーザーのスキルスコア集計値更新
    ...
    2. 上記1.と紐づくスキルクラスの把握
    3. 上記2.の各スコア更新処理
        - スキルユニットスコア (`skill_unit_scores.percentage`)
        - スキルクラススコアとレベル (`skill_class_scores.percentage`, `skill_class_scores.level`)
        - 上位スキルクラス開放 (上位の`skill_class_scores`生成)
            - 下位の`skill_class_scores.percentage`が40%を越えたもののみ対象
    4. 上記2.のスキルパネルをもつユーザーのキャリアフィールドスコアの更新処理
        - 素案：運用上のキャリアフィールドスコアの更新がバッチなのでそれを呼ぶ形
