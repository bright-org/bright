defmodule BrightWeb.BrightCoreComponents do
  ...
  def action_button(%{icon: nil} = assigns) do
    ~H"""
    <button type={@type} class={["text-sm font-bold px-5 py-2 rounded border border-base hover:opacity-50", @class]} {@rest}>
      <%= render_slot(@inner_block) %>
    </button>
    """
  end
  ...
  def action_button(assigns) do
    ...
    <button
      type={@type}
      class={["text-sm font-bold border border-base rounded py-1.5 pl-3 flex items-center hover:opacity-50", @class]}
      @rest
    >
      <%= render_slot(@inner_block) %>
      <span
