  ...
  def job_area(assigns) do
    ...
    <div class="w-[380px] text-start">
      ...
      <%= if @job.office_work do %>
        <p class="mb-2 ">
          <span>
            出勤：可<%= if @job.office_working_hours, do: "　#{@job.office_working_hours}" %>土日祝<%= enable?(
              @job.office_work_holidays
            ) %>
          </span>
        </p>
        <p class="mb-2">
      ...
      <p class="mb-2">
        <%= if @job.remote_work do %>
          <span>
            リモート：可<%= if @job.remote_working_hours, do: "　#{@job.remote_working_hours}" %>土日祝<%= enable?(
              @job.remote_work_holidays
            ) %>
          </span>
        <% else %>
          <span>リモート：不可</span>
    ...
    <div class="border-l border-brightGray-200 border-dashed ml-2 pl-2 w-28">
      <%= if @user.id in @stock_user_ids do %>
        <p class="mb-2 justify-center text-gray-300 bg-white px-2 py-1 inline-flex font-medium rounded-md text-sm items-center border border-gray-300 w-28 cursor-default">
          <span class="material-symbols-outlined md-18 mr-1 text-gray-300">inventory</span> ストック
        </p>
      <% else %>
        <button
          type="button"
          phx-click={
            JS.push("stock",
              value: %{
                params: %{
                  user_id: @user.id,
                  skill_panel: @skill_panel.skill_panel_name,
                  desired_income: @user.user_job_profile.desired_income
                }
              },
        ...
        >
          <span class="material-symbols-outlined md-18 mr-1 text-brightGray-600">inventory</span> ストック
        </button>
      <% end %>
      <.link
        ...
        href={
          if @anon,
            do:
              skill_panel_path(
                "graphs",
                %{id: @skill_panel.skill_panel},
                %{name_encrypted: encrypt_user_name(@user)},
                false,
                true
              ) <>
                "?class=#{@skill_panel.class}",
            else:
              skill_panel_path("graphs", %{id: @skill_panel.skill_panel}, @user, false, false) <>
                "?class=#{@skill_panel.class}"
        }
      >
        成長パネル
