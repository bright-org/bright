defmodule BrightWeb.LayoutComponents do
      ...
      <head>
        ...
        <script :if={@enable_google_tag_manager && Bright.Utils.Env.prod?()}>
          (function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':
              ...
              })(window,document,'script','dataLayer','GTM-P98L4WM');
        </script>
        ...
        <meta
          property="og:title"
          content="Bright｜過去と今、未来のスキルから、あなたの輝きを見える化します"
        />
        <meta
  ...
  def google_tag_manager_noscript(assigns) do
    ...
    <noscript>
      <iframe
        src="https://www.googletagmanager.com/ns.html?id=GTM-P98L4WM"
        height="0"
        width="0"
  ...
  def user_header(assigns) do
    ...
    <div
      id="user-header"
      class="sticky top-0 z-40 flex flex-col-reverse justify-between px-4 py-2 border-brightGray-100 border-b bg-white w-full lg:flex-row lg:items-center lg:px-10 lg:relative"
    >
      <div class="bg-white fixed bottom-0 left-0 p-2 lg:mr-2 lg:static lg:p-0 w-full lg:w-[440px]">
  ...
  def side_menu(assigns) do
    ...
    <aside class="relative z-50 lg:z-20 lg:w-[110px]">
      ...
      <label
        id="sp_navi_open"
        class="bg-white block cursor-pointer fixed h-10 ml-4 left-0 rounded top-2 w-10 z-[60] peer-checked:z-0 lg:hidden"
        for="sp_navi_input"
      >
        <span class="absolute bg-brightGray-300 block cursor-pointer h-[3px] left-1 top-1.5 w-8 before:bg-brightGray-300 before:block before:content-[''] before:cursor-pointer before:h-[3px] before:absolute before:top-3 before:w-8 after:bg-brightGray-300 after:block after:content-[''] after:cursor-pointer after:h-[3px] after:absolute after:top-6 after:w-8">
        </span>
      </label>
      <label
        id="sp_navi_background"
        for="sp_navi_input"
        class="cursor-pointer hidden peer-checked:block bg-pureGray-600/90 h-full fixed right-0 top-0 w-full z-20 -ml-2"
      >
      </label>
      <div class="lg:gap-y-10 pt-2 fixed bg-brightGray-900 min-h-svh h-full overflow-y-scroll lg:overflow-y-visible flex-col w-[110px] z-40 lg:flex lg:items-center lg:static hidden peer-checked:flex peer-checked:animate-fade-in-left">
        ...
        <ul class="grid lg:flex lg:flex-col lg:items-center lg:gap-y-2">
          ...
          <li>
            ...
            >
              <img src="/images/common/icons/logout.svg" alt="path image" /> ログアウト
            </.link>
          </li>
        </ul>
      </div>
