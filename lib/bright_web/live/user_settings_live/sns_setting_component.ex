    ...
    <li class="block">
      <div class="flex flex-col mt-8">
        <div
          ...
          id={
            if provider in linked_providers(@linked_user_social_auths),
              do: "user_settings_sns_linked_provider",
              else: "user_settings_sns_unlinked_provider_#{index}"
          }
          ...
          <%= if provider in linked_providers(@linked_user_social_auths) do %>
            <div class="w-full">
              <BrightWeb.UserAuthComponents.social_auth_button
                ...
                variant={to_string(provider)}
              >
                <%= UserSocialAuth.provider_name(provider) %>と連携解除する
              </BrightWeb.UserAuthComponents.social_auth_button>
              <span class="ml-4">
                <%= user_social_auth_by_provider(@linked_user_social_auths, provider).display_name %>で連携中
              </span>
            </div>
          <% else %>
            <div class="w-full">
              <BrightWeb.UserAuthComponents.social_auth_button
                href={
                  if provider in not_implemented_providers(), do: "#", else: ~p"/auth/#{provider}"
                }
                variant={to_string(provider)}
              >
