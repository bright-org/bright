defmodule BrightWeb.Admin.SkillClassLive.SkillClassFormComponent do
  ...
  def update(%{skill_class: skill_class} = assigns, socket) do
    changeset = SkillPanels.change_skill_class(skill_class)
  ...
  @impl true
  def handle_event("validate", %{"skill_class" => params}, socket) do
    changeset =
      socket.assigns.skill_class
      |> SkillPanels.change_skill_class(params)
      |> Map.put(:action, :validate)

    {:noreply, assign_form(socket, changeset)}
  end

  def handle_event("save", %{"skill_class" => params}, socket) do
  ...
  defp save_skill_class(socket, :edit_skill_class, params) do
    ...
    case SkillPanels.update_skill_class(skill_class, params) do
      {:ok, _skill} ->
        {:noreply,
         socket
